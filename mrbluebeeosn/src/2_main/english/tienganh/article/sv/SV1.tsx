...
export default function SV1(): React.JSX.Element {
	...
	<main className="image image2">
		...
		<article>
			...
			<p><strong>CỤM</strong> (không có S + V) và <strong>CÂU</strong> (có S + V).</p>
			...
			<p>Về mặt toán học cấu trúc, cả Clause và Sentence trong tiếng Anh vốn chung một công thức cốt lõi: <strong>S</strong> + <strong>V</strong> (<strong>Chủ ngữ</strong> + <strong>Vị ngữ</strong>). Một câu lớn thực chất được cấu tạo từ một hoặc nhiều câu nhỏ hợp lại. Cấu trúc bản chất là một, tại sao phải đẻ ra hai cái tên gọi khác nhau để làm khó người học?</p>
			...
			<p>Từ đây, mọi thành phần phụ thuộc trong câu sẽ được chia làm hai loại dựa vào việc nó có chứa S + V hay không:</p>
				...
				<ul className="list-square">
					...
					<li>Nếu không có S + V → Gọi là <strong>Phụ cụm</strong> (Dependent Phrase - DP).</li>
			
					<li>Nếu có chứa S + V → Gọi là <strong>Phụ câu</strong> (Dependent Clause - DC).</li>
			...
			<p>Khi một Phụ cụm hoặc một Phụ câu đi làm nhiệm vụ cho một từ loại nào đó (Danh từ, Tính từ, Trạng từ), chúng ta chỉ cần ghép tên chức năng với tên cấu trúc. Tư duy ngữ pháp lúc này sẽ được sắp xếp theo một trục phân cấp cực kỳ logic và tuyến tính:</p>
